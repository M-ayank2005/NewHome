    ...
    async function fetchWeather() {
        try {
            ...
            if (!parsedData ||
                ...
                let weatherApi = `https://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=${currentUserLocation}&days=3&aqi=no&alerts=no&lang=${lang}`;
                ...
                if (!parsedData.error) {
                    ...
                    const filteredData = {
                        ...
                        forecast: {
                            forecastday: parsedData.forecast.forecastday.map(day => ({
            ...
            function UpdateWeather() {
                ...
                // Populate 3-Day Forecast if available
                if (parsedData.forecast && parsedData.forecast.forecastday && parsedData.forecast.forecastday.length >= 3) {
                    const forecastContainer = document.getElementById("forecastContainer");
                    if (forecastContainer) {
                        for (let i = 1; i <= 2; i++) {
                            const dayData = parsedData.forecast.forecastday[i];
                            const dayEl = document.getElementById(`forecastDay${i}`);
                            if (dayEl && dayData) {
                                // Date formatting
                                const dateObj = new Date(dayData.date_epoch * 1000);
                                const dateStr = dateObj.toLocaleDateString(currentLanguage.replace('_', '-'), { weekday: 'short' });
                                dayEl.querySelector(".forecast-date").textContent = i === 1 ? "Tomorrow" : dateStr;

                                // Icon
                                const iconUrl = dayData.day.condition.icon.replace("//cdn.weatherapi.com/weather/64x64/", "https://cdn.weatherapi.com/weather/128x128/");
                                const iconEl = dayEl.querySelector(".forecast-icon");
                                iconEl.onerror = () => iconEl.src = './svgs/defaultWeather.svg';
                                iconEl.src = iconUrl;

                                // Temp
                                const minT = fahrenheitCheckbox.checked ? dayData.day.mintemp_f : dayData.day.mintemp_c;
                                const maxT = fahrenheitCheckbox.checked ? dayData.day.maxtemp_f : dayData.day.maxtemp_c;
                                
                                dayEl.querySelector(".f-min").textContent = `${Math.round(minT)}°`;
                                dayEl.querySelector(".f-max").textContent = `${Math.round(maxT)}°`;
                            }
                        }
                    }
                }
