...
async function initializeClock() {
    ...
    function initializeClockType() {
        ...
        // Map legacy values to new style names
        if (savedClockType === "analog") {
            clocktype = "material";
        } else if (savedClockType === "digital") {
            clocktype = "digital";
        } else if (savedClockType) {
            clocktype = savedClockType;
        } else {
            clocktype = "material"; // Default
        }
    ...
    function updateanalogclock() {
        ...
        // Also update Apple and Minimal clock hands with direct rotation
        const appleHour = document.getElementById("appleHour");
        const appleMinute = document.getElementById("appleMinute");
        const appleSecond = document.getElementById("appleSecond");
        const minimalHour = document.getElementById("minimalHour");
        const minimalMinute = document.getElementById("minimalMinute");
        const minimalSecond = document.getElementById("minimalSecond");

        if (appleHour) appleHour.style.transform = `rotate(${cumulativeHourRotation}deg)`;
        if (appleMinute) appleMinute.style.transform = `rotate(${cumulativeMinuteRotation}deg)`;
        if (appleSecond) appleSecond.style.transform = `rotate(${cumulativeSecondRotation}deg)`;
        if (minimalHour) minimalHour.style.transform = `rotate(${cumulativeHourRotation}deg)`;
        if (minimalMinute) minimalMinute.style.transform = `rotate(${cumulativeMinuteRotation}deg)`;
        if (minimalSecond) minimalSecond.style.transform = `rotate(${cumulativeSecondRotation}deg)`;
    ...
    function startDigitalClock() {
        if (!digitalIntervalId) {
            digitalIntervalId = setInterval(() => {
                if (clocktype === "digital") updatedigiClock();
                else if (clocktype === "flip") updateFlipClock();
            }, 1000);
        }
    }
    ...
    function updateFlipClock() {
        const hourformatstored = localStorage.getItem("hourformat");
        let hourformat = hourformatstored === "true";
        const now = new Date();
        ...
        const digits = { h1: h[0], h2: h[1], m1: m[0], m2: m[1] };
        const ids = { h1: 'flipHourTen', h2: 'flipHourOne', m1: 'flipMinTen', m2: 'flipMinOne' };

        for (const key of Object.keys(digits)) {
            const el = document.getElementById(ids[key]);
            if (el && digits[key] !== lastFlipValues[key]) {
                el.classList.add('flipping');
                setTimeout(() => {
                    el.querySelector('span').textContent = digits[key];
                    setTimeout(() => el.classList.remove('flipping'), 400);
                }, 200);
            }
        }
        lastFlipValues = { ...digits };

        // AM/PM
        const flipAmPm = document.getElementById('flipAmPm');
        if (flipAmPm) flipAmPm.textContent = hourformat ? period : '';
        ...
        if (flipDate) {
            ...
            let dayName = translations[currentLanguage]?.days?.[dayOfWeek] ?? translations["en"].days[dayOfWeek];
            flipDate.textContent = `${dayName.substring(0, 3)}, ${dayOfMonth}`;
        }

        // Greeting (same as digital)
        const greetingCheckbox = document.getElementById("greetingcheckbox");
        const isGreetingEnabled = localStorage.getItem("greetingEnabled") === "true";
        const dateElement = document.getElementById("date");
        if (clocktype === "flip" && isGreetingEnabled) {
            const newGreeting = getGreeting();
            if (newGreeting !== lastGreetingString) {
                dateElement.style.display = "block";
                dateElement.innerText = newGreeting;
        ...
        } else if (clocktype === "flip") {
            dateElement.style.display = "none";
        }
    }

    // Helper: is this an analog-type clock style?
    function isAnalogStyle(style) {
        return style === "material" || style === "apple" || style === "minimal" || style === "analog";
    }

    // Helper: is this a digital-type clock style?
    function isDigitalStyle(style) {
        return style === "digital" || style === "flip";
    }
    ...
    document.addEventListener("visibilitychange", function () {
        if (document.visibilityState === "visible") {
            if (isAnalogStyle(clocktype)) {
                startAnalogClock();
                updateDate();
            } else if (isDigitalStyle(clocktype)) {
                startDigitalClock();
            }
        } else {
            stopAnalogClock();
    ...
    function displayClock() {
        const allClocks = {
            material: document.getElementById("analogClock"),
            apple: document.getElementById("appleAnalogClock"),
            minimal: document.getElementById("minimalAnalogClock"),
            digital: document.getElementById("digitalClock"),
            flip: document.getElementById("flipClock")
        };

        // Handle legacy "analog" value
        const activeStyle = clocktype === "analog" ? "material" : clocktype;

        for (const [style, el] of Object.entries(allClocks)) {
            if (el) {
                el.style.display = style === activeStyle ? "block" : "none";
            }
        }
    }
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        greetingCheckbox.disabled = !isDigitalStyle(clocktype);

        // Update format/greeting field visibility based on clock type
        function updateDigitalFieldsVisibility() {
            const isDigital = isDigitalStyle(clocktype);
            if (isDigital) {
                timeformatField.classList.remove("inactive");
                greetingField.classList.remove("inactive");
                greetingCheckbox.disabled = false;
            } else {
                // timeformat applies to all clocks, keep enabled
                // greeting only for digital-type
                greetingField.classList.add("inactive");
                greetingCheckbox.disabled = true;
            }
        }
        updateDigitalFieldsVisibility();

        // ===== Clock Style Picker =====
        const clockStylePicker = document.getElementById("clockStylePicker");
        if (clockStylePicker) {
            const styleButtons = clockStylePicker.querySelectorAll(".clock-style-btn");
            ...
            styleButtons.forEach(btn => {
                btn.addEventListener("click", function () {
                    ...
                    // Set new clock type
                    clocktype = newStyle;
                    localStorage.setItem("clocktype", newStyle);

                    // Display the new clock
                    displayClock();

                    // Start appropriate clock
                    if (isAnalogStyle(newStyle)) {
                        startAnalogClock();
                        updateanalogclock();
                        resetDateDisplay();
                    } else if (newStyle === "digital") {
                        updatedigiClock();
                        startDigitalClock();
                    } else if (newStyle === "flip") {
                        lastFlipValues = { h1: '', h2: '', m1: '', m2: '' };
                        updateFlipClock();
                        startDigitalClock();
                    }

                    // Update visibility of digital-only settings
                    updateDigitalFieldsVisibility();
                });
            });
        }
