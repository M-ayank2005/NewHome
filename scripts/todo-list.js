...
function addtodoItem() {
    ...
    todoList[t] = { title: rawText, status: "pending", pinned: false, priority: priority }; // Add data to the JSON variable
    const li = createTodoItemDOM(t, rawText, "pending", false, priority); // Create List item
...
function createTodoItemDOM(id, title, status, pinned, priority) {
    let li = document.createElement("li");
    li.innerHTML = sanitizeInput(title); // Sanitize before rendering in DOM

    // Create and append priority indicator
    if (priority && priority !== "none") {
        const priorityDot = document.createElement("span");
        priorityDot.setAttribute("class", "todo-priority-dot priority-" + priority);
        li.insertBefore(priorityDot, li.firstChild);
    }
...
todoulList.addEventListener("click", (event) => {
    if (event.target.tagName === "LI" || (event.target.classList && event.target.classList.contains("todo-priority-dot"))) {
        ...
        li.classList.toggle("checked"); // Check the clicked LI tag
...
function ShowToDoList() {
    try {
        ...
        for (let id in todoList) {
            const todo = todoList[id];
            const li = createTodoItemDOM(id, todo.title, todo.status, todo.pinned, todo.priority || "none"); // Create `li` elements
...
function updateTodoBadge() {
    ...
    const completed = keys.filter(k => todoList[k].status === "completed").length;
    const pending = total - completed;

    // Badge
    if (todoBadge) {
        todoBadge.textContent = pending;
        todoBadge.style.display = pending > 0 ? "inline-flex" : "none";
    }

    // Progress bar
    if (todoProgressBar) {
        const pct = total > 0 ? Math.round((completed / total) * 100) : 0;
        todoProgressBar.style.width = pct + "%";
    }
}
